...
def train(train_dataloader, validation_dataloader, lm, save_dir_root, args):
    if args.model_type == 'gpt':
        ...
        return _train_bart(train_dataloader, validation_dataloader, lm, save_dir_root, args)

def _train_bart(train_dataloader, validation_dataloader, lm, save_dir_root, args):
    model = lm.model
    ...
    for _ in train_iterator:
        ...
        if _ % args.freq_save_epochs == 0:
            save_bart(model, tokenizer, save_dir_root, '%s/epoch%02d' % (args.model_name, _))
...
def validate(eval_dataloader, lm, model_type):
    ...
    if model_type == 'bart':
        return _validate_bart(eval_dataloader, lm, model_type)

def _validate_bart(eval_dataloader, lm, model_type):
    model = lm.model
    tokenizer = lm.tokenizer
    eval_loss = 0.0
    nb_eval_steps = 0
...
def save_bart(model, tokenizer, save_dir_root, name):
    output_dir = os.path.join(save_dir_root, "bart", name)
...
def parse_args():
    ...
    parser.add_argument('--model_type', default='bart', help='ngram | gpt | bart')
...
if __name__ == "__main__":
    ...
    if args.model_type == 'ngram':
        ...
        print("Attempting BART")
        model = BART2LM('facebook/bart-large')
    else:
