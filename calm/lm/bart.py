...
class BART2LM(BaseLM):
    ...
    def load_tokenizer(self):
        self.tokenizer = AutoTokenizer.from_pretrained('facebook/bart-large', cls_token='[CLS]', sep_token='[SEP]', model_max_length=1024)
    ...
    def generate(self, input, k, mask_out=ILLEGAL_ACTIONS + NO_EFFECT_ACTIONS, key=None):
        ...
        outputs = generate_topk(self.model, input_ids=input_ids, do_sample=False, num_beams=min(k * 2, 40),
                                ...
                                mask_out=mask_out)
        actions = [self.tokenizer.decode(output[input_len:], skip_special_tokens=False).split('[SEP]')[0].strip().lower() for output in outputs]
...
def _generate_beam_search_topk(
    ...
    while cur_len < max_length:
        ...
        outputs = self(model_inputs['decoder_input_ids'])  # (batch_size * num_beams, cur_len, vocab_size)
